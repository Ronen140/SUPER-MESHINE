...
- **אין דפדפן headless בסביבה (לא Playwright/Chromium) —** F1-F2 אומתו ב-`pnpm dev`+`curl` (HTML/redirects/CSS בפועל) ו-Vitest+Testing Library (jsdom — DOM אמיתי) לאינטראקציה (הקלדה, submit, ולידציה, ARIA), לא בדפדפן ויזואלי אמיתי. מומלץ סבב ידני בדפדפן אמיתי לפני production, בדגש RTL חזותי ו-console warnings.
